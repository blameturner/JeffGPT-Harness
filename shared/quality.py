...
from tools.search.urls import _strip_injection_patterns

_log = logging.getLogger("workers.quality")
...
VALIDATOR_CLASSIFIER_CHAR_BUDGET = get_function_config("content_quality").get("max_input_chars", 1500)
...
Answer: NAVIGATION
---
Content: In a landmark study published this week, scientists at Stanford reported that the new model outperformed baselines by 12 percent on standard benchmarks. The team trained on 400 billion tokens.
Answer: ARTICLE
---
...
def _classify_content_type(text: str) -> tuple[str | None, str, int]:
    ...
    raw, tokens = model_call("content_quality", prompt)
    if not raw:
        return None, "", 0
