...
tools.search.heuristics (_explicit_search_intent, _definitely_no_search)
and adds rag_lookup detection that the existing heuristics don't cover.
...
def gate_check(
    ...
    enable additional patterns for API lookups and error traces that the
    chat-focused heuristics miss.
    ...
    # Definitely-no-search still allows rag_lookup on short conversational turns.
