...
async def _summarise_one(
    ...
    if "deep" in function_name or "research" in function_name:
        prompt = (
            f"Analyse the following web page and extract ALL information relevant to: {user_query}\n\n"
            f"YOUR OUTPUT MUST INCLUDE (in this order):\n\n"
            f"1. FACTS: Specific data points, statistics, numbers, dates, prices, percentages. "
            f"Quote them exactly as they appear in the source.\n\n"
            f"2. CLAIMS: Main arguments or conclusions made by the author. "
            f"Attribute each claim (e.g. 'According to [author/org]...').\n\n"
            f"3. EVIDENCE QUALITY: Is this a primary source (official data, research paper, documentation) "
            f"or secondary (blog, forum, opinion piece)? How recent is the data?\n\n"
            f"4. CAVEATS: Any limitations, biases, or missing context noted in the source.\n\n"
            f"Keep your analysis under 500 words. Be specific — extract actual data, not summaries of data.\n\n"
            f"On the VERY LAST line of your response, output ONLY this tag:\n"
            f"RELEVANCE: high|medium|low\n"
            f"(high = directly answers the query with specific data, "
            f"medium = related context but no direct answer, "
